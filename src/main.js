...
function applyAll() {
  ...
  r.setProperty('--border-dim', adjustBrightness(settings.colBorder, -18));
  r.setProperty('--border-bright', adjustBrightness(settings.colBorder, 20));
  r.setProperty('--border-white', adjustBrightness(settings.colBorder, 40));
  r.setProperty('--accent', settings.colAccent);
  r.setProperty('--text-dim', settings.colTextDim || DEFAULTS.colTextDim);
  r.setProperty('--text-mid', settings.colTextMid || DEFAULTS.colTextMid);
  r.setProperty('--text-base', settings.colTextBase || DEFAULTS.colTextBase);
  r.setProperty('--text-bright', settings.colTextBright || DEFAULTS.colTextBright);
  r.setProperty('--text-white', settings.colTextWhite || DEFAULTS.colTextWhite);
  r.setProperty('--scanline-opacity', String(Math.max(0, Math.min(0.2, Number(settings.scanlineOpacity ?? DEFAULTS.scanlineOpacity)))));
...
const THEME_PRESETS = {
  'high-contrast': {
    colBg: '#050505', colAccent: '#8cd2ff', colBorder: '#4a4a4a',
    colTextDim: '#9a9a9a', colTextMid: '#c0c0c0', colTextBase: '#dbdbdb',
    colTextBright: '#f5f5f5', colTextWhite: '#ffffff', scanlineOpacity: 0.04,
  },
  'light-mode': {
    colBg: '#f4f6fb', colAccent: '#2563eb', colBorder: '#a7b0c0',
    colTextDim: '#5f6b7c', colTextMid: '#3f4a5b', colTextBase: '#2f3846',
    colTextBright: '#1f2630', colTextWhite: '#11151c', scanlineOpacity: 0.0,
  },
  'cyber-blue': {
    colBg: '#041019', colAccent: '#59d8ff', colBorder: '#23506b',
    colTextDim: '#7ea7bf', colTextMid: '#a9ccdf', colTextBase: '#cae5f1',
    colTextBright: '#e8f8ff', colTextWhite: '#ffffff', scanlineOpacity: 0.03,
  },
  'matrix-green': {
    colBg: '#030a03', colAccent: '#6dff99', colBorder: '#2a5b2a',
    colTextDim: '#78a278', colTextMid: '#9fd09f', colTextBase: '#bde7bd',
    colTextBright: '#ddffdd', colTextWhite: '#f3fff3', scanlineOpacity: 0.05,
  },
  'amber-terminal': {
    colBg: '#120d05', colAccent: '#ffc46b', colBorder: '#6e4d1f',
    colTextDim: '#b39667', colTextMid: '#d1b07c', colTextBase: '#e4c996',
    colTextBright: '#f6dfb3', colTextWhite: '#fff3dd', scanlineOpacity: 0.04,
  },
};
...
const App = {
  ...
  openSettings() {
    ...
    sv('s-scanline-opacity', settings.scanlineOpacity ?? DEFAULTS.scanlineOpacity);
    ...
    sv('s-col-text-dim', settings.colTextDim || DEFAULTS.colTextDim);
    sv('s-col-text-dim-hex', settings.colTextDim || DEFAULTS.colTextDim);
    sv('s-col-text-mid', settings.colTextMid || DEFAULTS.colTextMid);
    sv('s-col-text-mid-hex', settings.colTextMid || DEFAULTS.colTextMid);
    sv('s-col-text-bright', settings.colTextBright || DEFAULTS.colTextBright);
    sv('s-col-text-bright-hex', settings.colTextBright || DEFAULTS.colTextBright);
  ...
  async saveSettings() {
    ...
    settings.colTextDim   = gv('s-col-text-dim');
    settings.colTextMid   = gv('s-col-text-mid');
    settings.colTextBright= gv('s-col-text-bright');
    settings.colTextBase  = adjustBrightness(settings.colTextMid, 28);
    settings.colTextWhite = adjustBrightness(settings.colTextBright, 24);
    settings.scanlineOpacity = Math.max(0, Math.min(0.2, Number(gv('s-scanline-opacity') || DEFAULTS.scanlineOpacity)));
  ...
  applyUISettings() {
    ...
    const tdm = gv('s-col-text-dim');
    const tmd = gv('s-col-text-mid');
    const tbr = gv('s-col-text-bright');
    const sl  = Math.max(0, Math.min(0.2, Number(gv('s-scanline-opacity') || DEFAULTS.scanlineOpacity)));
    const r   = document.documentElement.style;
    ...
    if(brd) {
      r.setProperty('--border',brd);
      r.setProperty('--border-dim', adjustBrightness(brd, -18));
      r.setProperty('--border-bright', adjustBrightness(brd, 20));
      r.setProperty('--border-white', adjustBrightness(brd, 40));
    }
    if(tdm) r.setProperty('--text-dim', tdm);
    if(tmd) {
      r.setProperty('--text-mid', tmd);
      r.setProperty('--text-base', adjustBrightness(tmd, 28));
    }
    if(tbr) {
      r.setProperty('--text-bright', tbr);
      r.setProperty('--text-white', adjustBrightness(tbr, 24));
    }
    r.setProperty('--scanline-opacity', String(sl));
  ...
  applyThemePreset() {
    ...
    sv('s-col-bg', cfg.colBg); sv('s-col-bg-hex', cfg.colBg);
    sv('s-col-accent', cfg.colAccent); sv('s-col-accent-hex', cfg.colAccent);
    sv('s-col-border', cfg.colBorder); sv('s-col-border-hex', cfg.colBorder);
    sv('s-col-text-dim', cfg.colTextDim); sv('s-col-text-dim-hex', cfg.colTextDim);
    sv('s-col-text-mid', cfg.colTextMid); sv('s-col-text-mid-hex', cfg.colTextMid);
    sv('s-col-text-bright', cfg.colTextBright); sv('s-col-text-bright-hex', cfg.colTextBright);
    sv('s-scanline-opacity', String(cfg.scanlineOpacity));
    App.applyUISettings();
  },

  autoContrast() {
    const bg = gv('s-col-bg') || DEFAULTS.colBg;
    const lum = relativeLuminance(bg);
    const darkModeBg = lum < 0.42;

    const border = darkModeBg ? adjustBrightness(bg, 46) : adjustBrightness(bg, -42);
    const textDim = darkModeBg ? '#8f98a3' : '#5f6b7c';
    const textMid = darkModeBg ? '#bcc6d2' : '#3f4a5b';
    const textBright = darkModeBg ? '#f5f8ff' : '#1f2630';
    let accent = gv('s-col-accent') || DEFAULTS.colAccent;

    if (contrastRatio(accent, bg) < 3) {
      accent = darkModeBg ? '#7fc5ff' : '#2563eb';
    }

    sv('s-col-border', border); sv('s-col-border-hex', border);
    sv('s-col-text-dim', textDim); sv('s-col-text-dim-hex', textDim);
    sv('s-col-text-mid', textMid); sv('s-col-text-mid-hex', textMid);
    sv('s-col-text-bright', textBright); sv('s-col-text-bright-hex', textBright);
    sv('s-col-accent', accent); sv('s-col-accent-hex', accent);

    if (darkModeBg && Number(gv('s-scanline-opacity') || 0) > 0.08) {
      sv('s-scanline-opacity', '0.06');
    }
    if (!darkModeBg) {
      sv('s-scanline-opacity', '0.00');
    }

    sv('s-theme-preset', 'custom');
    App.applyUISettings();
    addLog('sys', 'Auto contrast applied for current background.');
  },
  ...
  resetColors() {
    ...
    sv('s-col-text-dim',DEFAULTS.colTextDim); sv('s-col-text-dim-hex',DEFAULTS.colTextDim);
    sv('s-col-text-mid',DEFAULTS.colTextMid); sv('s-col-text-mid-hex',DEFAULTS.colTextMid);
    sv('s-col-text-bright',DEFAULTS.colTextBright); sv('s-col-text-bright-hex',DEFAULTS.colTextBright);
    sv('s-scanline-opacity', String(DEFAULTS.scanlineOpacity));
    sv('s-theme-preset', 'custom');
    App.applyUISettings();
  },
  ...
  connectNext() {
    if (connectState.step === 1) return App.connectStep2();
    if (connectState.step === 2) return App.connectStep3();
  },
...
window.syncModelInput = syncModelInput;
window.updatePlatformFields = updatePlatformFields;
