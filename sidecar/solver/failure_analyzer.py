...
_FAILURE_CLASS_LIBRARY = {
    ...
    "null_dereference": {
        "transforms": ["guard_null_ptr", "validate_address", "add_precondition_check"],
        "priority": 0.8,
    },
    "integer_overflow": {
        "transforms": ["check_arithmetic_bounds", "use_safe_add", "mask_result"],
        "priority": 0.75,
    },
    "format_string_crash": {
        "transforms": ["sanitize_format_arg", "trace_printf_args", "align_arg_count"],
        "priority": 0.78,
    },
    "heap_corruption": {
        "transforms": ["fix_chunk_alignment", "add_heap_trace", "use_safe_free"],
        "priority": 0.88,
    },
    "authentication_failure": {
        "transforms": ["rotate_credentials", "try_default_creds", "inspect_auth_flow"],
        "priority": 0.65,
    },
    "network_refused": {
        "transforms": ["check_port", "retry_with_backoff", "verify_host"],
        "priority": 0.6,
    },
    "import_error": {
        "transforms": ["install_missing_dep", "use_stdlib_alternative", "mock_import"],
        "priority": 0.55,
    },
    "decode_error": {
        "transforms": ["try_alternate_encoding", "strip_bom", "handle_partial_data"],
        "priority": 0.65,
    },
    "unknown": {
...
def _regex_classify(output: str) -> str:
    ...
    if "stack smashing" in low or "canary" in low:
        return "stack_canary"
    ...
    if "traceback" in low or "syntaxerror" in low:
        return "exploit_runtime_error"
    if "null pointer" in low or "nullpointer" in low:
        return "null_dereference"
    if "heap" in low and ("corrupted" in low or "double free" in low):
        return "heap_corruption"
    if "modulenotfounderror" in low or "importerror" in low:
        return "import_error"
    if "unicodedecode" in low or "invalid byte" in low:
        return "decode_error"
