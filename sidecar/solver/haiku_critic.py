...
def run_haiku_critic(model_reasoning: str, tool_results: list[dict], current_hypothesis: str, iteration: int, api_key: str = "") -> dict[str, Any]:
    default = {
...
def grade_tool_result(tool_name: str, result_text: str, category: str = "", api_key: str = "") -> dict[str, Any]:
    default = {
        "quality": 0.5,
        "extractable_facts": [],
        "noise_ratio": 0.3,
    ...
    flag_signal = any(
        tok in result_lower
        for tok in ["flag{", "ctf{", "picoctf{", "htb{", "thm{", "key:", "secret:", "password:", "token:", "-----begin"]
    )
    ...
    try:
        client = _haiku_client(api_key)
        user_msg = f"Tool: {tool_name} | Category: {category}\nOutput (first 1200 chars):\n{str(result_text)[:1200]}"
        resp = client.messages.create(
            model=_MODEL_HAIKU,
            max_tokens=256,
