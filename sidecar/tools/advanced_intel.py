...
def tool_symbolic_pipeline(binary_path: str = "", crypto_equations: list[str] | None = None,
    ...
    constraints = []
    constraints.extend(str(x) for x in _to_list(crypto_equations))
    constraints.extend(str(x) for x in _to_list(protocol_states))
    orchestrated = symbolic_orchestrate(
        challenge_type="reverse" if binary_path else "crypto",
...
def tool_generate_exploit_script(target: str = "", vuln_type: str = "auto",
    ...
    # Opus/Sonnet should synthesize payload code; this tool returns a structured generation spec.
    return _safe_json({
        "delegate_to_llm": True,
        "model_hint": "opus_or_sonnet",
        "task": "generate_exploit_script",
        "target": target,
        "vuln_type": vuln_type,
        "connection": {"host": host, "port": port},
        "requirements": [
            "Include leak -> resolve -> exploit phases",
            "Add retry and timeout handling",
            "Emit telemetry markers for reflection loop"
        ],
    })
...
def tool_generate_decoder(format_hint: str = "auto", sample: str = "") -> str:
    return _safe_json({
        "delegate_to_llm": True,
        "model_hint": "sonnet",
        "task": "generate_decoder",
        "format_hint": format_hint,
        "sample": sample,
        "requirements": ["multi-decoder strategy", "ranked outputs", "confidence per decoding"],
    })
...
def tool_generate_emulator(isa_name: str = "custom_vm", opcodes: dict | None = None) -> str:
    ...
    return _safe_json({
        "delegate_to_llm": True,
        "model_hint": "opus",
        "task": "generate_emulator",
        "isa": isa_name,
        "opcode_map": opcodes,
        "requirements": ["trace mode", "state diff output", "hookable I/O callbacks"],
    })
...
def tool_generate_patch(target: str = "", patch_type: str = "nop", offset: int = 0,
    ...
    return _safe_json({
        "delegate_to_llm": True,
        "model_hint": "sonnet",
        "task": "generate_patch_plan",
        "target": target,
        "offset": offset,
        "patch_type": patch_type,
        "replacement_hex": replacement_hex,
        "steps": ["backup binary", "propose patch bytes", "validate semantics", "re-run + compare behavior"]
    })
...
def tool_attack_path_finder(graph: dict | None = None, start: str = "", goal: str = "") -> str:
    graph = graph or {}
    found = a_star_attack_path(
        graph=graph,
        start=start,
        goal=goal,
        capabilities=set(str(x) for x in graph.get("capabilities", [])),
        beam_width=int(graph.get("beam_width", 64) or 64),
    )
    return _safe_json(found)
...
def tool_strategy_optimizer(elapsed_min: float = 0.0, iterations: int = 0,
    ...
    # Unified EV-style score packet for strategy selection.
    ranked = rank_with_unified_scorer([
        {
            "name": mode,
            "evidence": min(1.0, success_rate + 0.15),
            "novelty": 0.6 if mode in ("symbolic-heavy", "writeup+search") else 0.35,
            "exploitability": 0.55 if failures < 8 else 0.35,
            "success_prior": max(0.05, success_rate),
            "confidence": 0.6,
            "estimated_cost": max(0.1, elapsed_min / 60.0),
        }
    ])
    return _safe_json({
        "mode": mode,
...
def tool_branch_knowledge_share(branch_results: list[dict] | None = None) -> str:
    ...
    for br in branch_results:
        ...
        for k in ["leak", "offset", "key", "endpoint", "primitive"]:
            if k in br:
                facts.append({"key": k, "value": br[k], "confidence": float(br.get("confidence", 0.6)), "provenance": br.get("provenance", "branch")})
        normalized.append({"branch": br.get("branch", "unknown"), "timestamp": float(br.get("timestamp", now)), "facts": facts})
    return _safe_json(fuse_branch_results(normalized, half_life_min=90.0))
...
def tool_solution_merger(candidates: list[dict] | None = None, key: str = "score") -> str:
    ...
    ranked = rank_with_unified_scorer([
        {
            "candidate": c,
            "evidence": float(c.get("evidence", c.get(key, 0.0))),
            "novelty": float(c.get("novelty", 0.4)),
            "exploitability": float(c.get("exploitability", 0.5)),
            "success_prior": float(c.get("success_prior", 0.5)),
            "confidence": float(c.get("confidence", 0.6)),
            "estimated_cost": float(c.get("estimated_cost", 0.3)),
        }
        for c in candidates
    ])
...
def tool_ctf_pattern_classifier(text: str = "", source: str = "") -> str:
    corpus = (text + " " + source).strip()
    db_path = "~/.ctf-solver/signatures.json"
    try:
        db = load_signature_db(db_path)
        preds = predict_signatures(db, corpus, top_k=5)
        return _safe_json({"source": "data-driven", "predictions": preds})
    except Exception:
        pass
    return _safe_json({"source": "fallback", "predictions": []})
...
def tool_auto_exploit_loop(target: str = "", vuln_type: str = "auto", host: str = "",
                           port: int = 0, max_rounds: int = 5, script: str = "") -> str:
    if not script.strip():
        return _safe_json({
            "status": "needs_llm_script",
            "message": "Provide exploit script from Opus/Sonnet in 'script' field.",
            "context": {"target": target, "vuln_type": vuln_type, "host": host, "port": port},
        })
    loop = autonomous_exploit_loop(initial_script=script, rounds=max_rounds, timeout_s=12)
    return _safe_json(loop)
