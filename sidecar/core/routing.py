...
def route_model_v2(category: str, difficulty: str, iteration: int, total_iters: int,
                   ...
                   state_vector: dict | None = None) -> dict:
    profile = get_profile(category, learned_overrides=learned_overrides)
    hard_categories = {"Cryptography", "Reverse Engineering", "Web", "Forensics"}
    ...
    hard_threshold = int(profile.get("route_escalate_score", 62))
    soft_threshold = int(profile.get("route_soft_escalate_score", 48))
    ...
    if not use_opus and opus_budget_remaining == 0 and difficulty in ("hard", "insane"):
        use_thinking = True
        thinking_tokens = 8000 if difficulty == "hard" else 12000
        reasons.append("sonnet-thinking-fallback")
    elif not use_opus and route_score >= soft_threshold and difficulty == "hard":
        use_thinking = True
        thinking_tokens = max(thinking_tokens, 6000)
        reasons.append("sonnet-thinking-soft-escalate")
