...
Enhancement 5: Before the first expensive Opus/Sonnet call, runs a cheap
Haiku pass to generate 5 candidate attack hypotheses in parallel and return
them ranked by confidence. Seeds the branch pool with higher-quality
initial branches.
"""
...
def speculative_hypothesis_fanout(
    challenge_description: str,
    category: str,
    difficulty: str = "medium",
    rag_context: str = "",
    cross_ctf_context: str = "",
    api_key: str = "",
) -> list[dict[str, Any]]:
    """
    Run a Haiku pre-analysis to generate 5 ranked hypotheses before main solve.

    Returns list of hypothesis dicts sorted by confidence descending.
    Falls back to generic category hypotheses if the API call fails.
    """
    if not api_key:
    ...
    try:
        ...
        user_msg = (
            ...
            f"Challenge description:\n{str(challenge_description)[:2000]}"
            f"{context_block}"
        )
...
def _generic_fallback(category: str, difficulty: str) -> list[dict[str, Any]]:
    """Category-aware generic hypotheses when Haiku is unavailable."""
...
def render_fanout_for_prompt(hypotheses: list[dict[str, Any]]) -> str:
    """Format hypothesis fan-out results for system prompt injection."""
    if not hypotheses:
        return ""
    lines = ["## Pre-solve hypothesis fan-out (Haiku pre-analysis — ranked by confidence):"]
    for h in hypotheses:
        lines.append(
            f"  #{h.get('rank', '?')} [{h.get('confidence', 0):.0%}] {h.get('hypothesis', '')}"
        )
        if h.get("first_tool"):
