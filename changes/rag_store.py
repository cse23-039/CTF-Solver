...
def _embed_text(text: str, dim: int = 128) -> list[float]:
    ...
    # Hash-projection fallback — stable, fast, ~0.6 Pearson correlation
    # with true embeddings for keyword-heavy technical text.
    vec = [0.0] * dim
...
class RAGStore:
    ...
    def ingest(self, record: dict[str, Any]) -> None:
        """
        Store a solved challenge with its embedding.

        Required keys: ctf_name, challenge_name, category, difficulty,
                       description_text, attack_technique, winning_tool_sequence,
                       solve_summary.
        """
    ...
    def query(self, description: str, category: str = "",
              top_k: int = 3, min_similarity: float = 0.25) -> list[dict[str, Any]]:
        """
        Retrieve the top-k most similar solved challenges.

        Returns list of dicts with keys: ctf_name, challenge_name, category,
        difficulty, attack_technique, winning_tool_sequence, solve_summary, similarity.
        """
        query_text = f"{description} {category}".strip()
        q_emb = _embed_text(query_text)

        with self._conn() as conn:
            # Limit candidate scan to 2000 most recent rows for speed.
            rows = conn.execute("""
    ...
    def render_context_for_prompt(self, results: list[dict[str, Any]]) -> str:
        """Render RAG results as a system-prompt injection block."""
        if not results:
            return ""
    ...
    def count(self) -> int:
        with self._conn() as conn:
            return conn.execute("SELECT COUNT(*) FROM writeup_embeddings").fetchone()[0]
...
def ingest_solved_challenge(record: dict[str, Any]) -> None:
    """Public helper called from engine after a successful solve."""
    try:
...
def retrieve_similar_challenges(description: str, category: str = "",
                                 top_k: int = 3) -> list[dict[str, Any]]:
    """Public helper called from engine before the first API call."""
    try:
...
def render_rag_context(description: str, category: str = "", top_k: int = 3) -> str:
    """One-shot: retrieve + render for system prompt injection."""
